// DETOXY HIJAMA — Shared Nav, Footer, Cart, Utils
// Include AFTER config.js

// ── SOCIAL LINKS ──
const SOCIAL = {
  wa:       `https://wa.me/${DETOXY_CONFIG.WHATSAPP_NUMBER}`,
  waBulk:   `https://wa.me/${DETOXY_CONFIG.WHATSAPP_BULK}`,
  linkedin: 'https://www.linkedin.com/company/detoxy-hijama',
  instagram:'https://www.instagram.com/detoxyhijama_',
  facebook: 'https://www.facebook.com/detoxyhijama',
  youtube:  'https://www.youtube.com/@detoxyhijama',
};

// SVG Icons
const ICONS = {
  wa: `<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>`,
  linkedin: `<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>`,
  cart: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>`,
};

// Cart utility (persists to localStorage)
const Cart = {
  _key: 'dh_cart_v2',
  get()   { try { return JSON.parse(localStorage.getItem(this._key)||'[]'); } catch(e){return[];} },
  save(items){ localStorage.setItem(this._key, JSON.stringify(items)); this.updateBadge(); },
  add(product){
    const items = this.get();
    const i = items.findIndex(x=>x.id===product.id);
    if(i>-1) items[i].qty += (product.qty||1);
    else items.push({...product, qty:product.qty||1});
    this.save(items);
    this.toast(`${product.name} added to cart!`);
  },
  remove(id){ this.save(this.get().filter(i=>i.id!==id)); },
  clear(){ this.save([]); },
  count(){ return this.get().reduce((s,i)=>s+i.qty,0); },
  total(){ return this.get().reduce((s,i)=>s+i.price*i.qty,0); },
  updateBadge(){
    document.querySelectorAll('.cart-badge').forEach(b=>{
      const c=this.count(); b.textContent=c; b.style.display=c>0?'flex':'none';
    });
  },
  toast(msg){
    let t=document.getElementById('dhToast');
    if(!t){
      t=document.createElement('div'); t.id='dhToast';
      t.style.cssText='position:fixed;bottom:90px;right:24px;background:#0b1f1c;color:#fff;padding:13px 20px;border-radius:12px;font-size:.84rem;z-index:9999;box-shadow:0 8px 32px rgba(0,0,0,.35);opacity:0;transition:opacity .3s,transform .3s;transform:translateY(10px);font-family:Outfit,sans-serif;border-left:3px solid #2aab97;max-width:300px';
      document.body.appendChild(t);
    }
    t.textContent='🛒 '+msg; t.style.opacity='1'; t.style.transform='translateY(0)';
    clearTimeout(t._t); t._t=setTimeout(()=>{t.style.opacity='0';t.style.transform='translateY(10px)';},3000);
  }
};

document.addEventListener('DOMContentLoaded', ()=>Cart.updateBadge());
