...
<header class="site-header" id="siteHeader">
  <a href="${b}index.html" class="brand">
    <div class="brand-logo-wrap">
      <img src="${b}assets/logo.png" class="brand-logo-img" alt="Detoxy Hijama" onerror="this.style.display='none'">
      <svg class="brand-logo-svg" width="36" height="36" viewBox="0 0 36 36" fill="none">
        <circle cx="18" cy="18" r="18" fill="#2aab97"/>
        <path d="M18 8c0 0-7 4.5-7 10.5 0 3.87 3.13 7 7 7s7-3.13 7-7C25 12.5 18 8 18 8z" fill="white" opacity="0.9"/>
        <circle cx="18" cy="20" r="3" fill="#2aab97"/>
      </svg>
    </div>
    <div class="brand-name-wrap">
      <span class="brand-main">Detoxy Hijama</span>
      <span class="brand-sub">Manufacturer · Direct</span>
    </div>
  </a>
  ...
  <div class="header-actions">
    ...
    <a href="${b}admin/login.html" class="login-btn" title="Admin Login" id="loginBtnHeader">
...
<nav class="mob-nav" id="mobNav">
  ...
  <a href="${b}admin/login.html" class="mob-link">👤 Admin Login</a>
  <div style="margin-top:24px;padding-top:20px;border-top:1px solid rgba(255,255,255,.08)">
    <a href="https://wa.me/919566596077" style="display:flex;align-items:center;justify-content:center;gap:10px;padding:12px 20px;background:#25d366;border-radius:12px;color:#fff;font-weight:700;font-size:.88rem;text-decoration:none" target="_blank">
      ${_WA_SVG} WhatsApp Us
    </a>
  </div>
</nav>
<style>
.brand-logo-wrap{position:relative;width:36px;height:36px;flex-shrink:0}
.brand-logo-img{width:36px;height:36px;border-radius:50%;object-fit:cover;position:absolute;inset:0;z-index:2}
...
window.Cart = {
  ...
  init() {
    this._updateBadge();
    // Official WhatsApp float button
    if (!document.getElementById('_wa_float')) {
      ...
      wa.style.cssText = 'position:fixed;bottom:24px;right:24px;width:56px;height:56px;border-radius:50%;background:#25d366;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(37,211,102,.5);z-index:888;transition:transform .2s,box-shadow .2s;text-decoration:none';
      wa.onmouseover = () => { wa.style.transform='scale(1.1)'; wa.style.boxShadow='0 8px 32px rgba(37,211,102,.65)'; };
      wa.onmouseleave = () => { wa.style.transform='scale(1)'; wa.style.boxShadow='0 4px 20px rgba(37,211,102,.5)'; };
...
window.OrderSync = {
  ...
  async submit(orderData) {
    ...
    if (!url || url.includes('YOUR_SCRIPT_ID') || url.trim() === '') {
      console.warn('[OrderSync] Google Sheets URL not configured. Order saved locally only.');
      return { success: true, local: true };
    }

    const result = await this._sendToSheets(url, orderData);
    if (!result) {
      // Queue for retry
      this._queueForRetry(orderData);
    }
    return { success: true };
  },

  async _sendToSheets(url, data) {
    try {
      await fetch(url, {
        method: 'POST',
        mode: 'no-cors',
      ...
      console.error('[OrderSync] Sheets sync failed:', e.message);
      return false;
    }
  },

  _queueForRetry(orderData) {
    const q = JSON.parse(localStorage.getItem(this._queueKey) || '[]');
    if (!q.find(o => o.id === orderData.id)) q.push(orderData);
  ...
  async retryQueue() {
    ...
    for (const order of q) {
      const ok = await this._sendToSheets(url, order);
      if (!ok) remaining.push(order);
    }
    localStorage.setItem(this._queueKey, JSON.stringify(remaining));
    if (remaining.length < q.length) console.log(`[OrderSync] Retried ${q.length - remaining.length} queued orders.`);
  },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Retry any failed Sheets syncs silently on page load
